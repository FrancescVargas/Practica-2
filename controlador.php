...
$c["bd"]=function()  
{
    $pdo=new PDO("mysql:host=localhost;dbname=cinemania","root");
...
$app->post("/publicoment", function($request,$response,$args)
          {
            ...
            $sql="insert into criticas(autor,id_pelicula,texto,nota) values('${params["autor"]}','${params["idpelis"]}','${params["texto"]}','${params["nota"]}');";
            $res=$con->exec($sql);
              
            return $response->withRedirect('/Francesc/Practica-2/controlador.php/publicvistapeli?id='.$params['idpelis']);
